...
int main(void) {
  ...
  I2C_HOLD;
  // turn on oscillator
  i2c_register_write(&port, ADDRESS, DPMODE1, 0x00);
  // outputs change on ACK and not on STOP
  i2c_register_write(&port, ADDRESS, DPMODE2, 0x0D);
  // turn on all LED drivers
  for (int i = 0; i < 4; i++)
    i2c_register_write(&port, ADDRESS, DPLEDOUT(i), DPLDR_PWM_ALL);
  i2c_stop(&port);
  ...
  while(brightness[15] < 255) {
    for (int i = 0; i < 16; i++) {
      if ((i == 0) || (i > 0 && brightness[i-1] > 50)) {
        if (brightness[i] < 255) {
          brightness[i] ++;
          i2c_register_write(&port, ADDRESS, DPPWM(i), brightness[i]);
        }
      }
    }
    _delay_ms(2);
  ...
  i2c_register_addr(&port, ADDRESS, DPAUTOINCR_PWM | DPPWM(0));
  for(int b = 255; b > 0; b --) {
    for(int i = 0; i < 16; i++)
      i2c_write(&port, b);
    _delay_us(100);
  ...
  for (int i = 15; i >= 0; i--) {
    for (int j = 0; j <= i; j ++) {
      if (j > 0) i2c_register_write(&port, ADDRESS, DPPWM(j-1), 0x00);
      i2c_register_write(&port, ADDRESS, DPPWM(j), 0xFF);
      i2c_stop(&port);
      _delay_ms(22);
    }
  }
  ...
  // pseudo-randomness
  for (int i = 0; i < 255; i++) {
    i2c_register_write(&port, ADDRESS, DPPWM((uint8_t)(xor128() % (uint32_t)16)), (uint8_t)(xor128() % (uint32_t)16));
    _delay_ms(10);
  }
  i2c_stop(&port);
  info();
  ...
  while (brightness[15] > 0) {
    for (int i = 0; i < 16; i++) {
      if ((i == 0) || (i > 0 && brightness[i-1] < 200)) {
        if (brightness[i] > 0) {
          brightness[i] --;
          if (! i2c_register_write(&port, ADDRESS, DPPWM(i), brightness[i])) fail();
        }
      }
    }
    i2c_stop(&port);
